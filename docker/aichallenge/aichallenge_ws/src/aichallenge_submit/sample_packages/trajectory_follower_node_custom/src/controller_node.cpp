...
Controller::Controller(const rclcpp::NodeOptions & node_options) : Node("controller", node_options)
{
  ...
  sub_simple_pure_pursuit_cmd_ = create_subscription<autoware_auto_control_msgs::msg::AckermannControlCommand>(
    "~/input/simple_pure_pursuit_cmd", rclcpp::QoS{1},
    [this](const autoware_auto_control_msgs::msg::AckermannControlCommand::SharedPtr msg) { simple_pure_pursuit_cmd_ptr_ = msg; });
  sub_rl_planner_cmd_ = create_subscription<autoware_auto_control_msgs::msg::AckermannControlCommand>(
    "/control/rl_planner/control_cmd", rclcpp::QoS{1},
    [this](const autoware_auto_control_msgs::msg::AckermannControlCommand::SharedPtr msg) {rl_planner_cmd_ptr_ = msg;});
  sub_manual_cmd_ = create_subscription<autoware_auto_control_msgs::msg::AckermannControlCommand>(
    "/manual_cmd", rclcpp::QoS{1},
    [this](const autoware_auto_control_msgs::msg::AckermannControlCommand::SharedPtr msg) { manual_cmd_ptr_ = msg; });
...
{
  // RCLCPP_INFO(get_logger(), "start call back");
  // 1. create input data
  ...
  // RCLCPP_INFO(get_logger(), "input ok");
  ...
  // RCLCPP_INFO(get_logger(), "controller runed");

  // 4. sync with each other controllers
  longitudinal_controller_->sync(lat_out.sync_data);
  lateral_controller_->sync(lon_out.sync_data);
  // RCLCPP_INFO(get_logger(), "controller synced");
  ...
  out.stamp = this->now();
  // 基本はsimple pure pursuitを使用
  if(simple_pure_pursuit_cmd_ptr_) {
    out = *simple_pure_pursuit_cmd_ptr_;
  } else {
    RCLCPP_INFO(get_logger(), "pure_pursuit not ready");
    return;
  }
  // MPC解が得られている場合は、横制御のみMPCを使用
  if (lateral_controller_->isSolved()) {
    out.lateral = lat_out.control_cmd;
    // RCLCPP_INFO(get_logger(), "MPC is appled");
  } else {
    RCLCPP_INFO(get_logger(), "MPC is not appled");
  }
  // 強化学習プランナからの入力
  if (rl_planner_cmd_ptr_) {
    out.longitudinal = rl_planner_cmd_ptr_->longitudinal;
    // RCLCPP_INFO(get_logger(), "rl_planner_cmd is appled");
  }
  // マニュアル入力
  if (manual_cmd_ptr_) {
    RCLCPP_INFO(get_logger(), "manual input!");
    out.lateral = manual_cmd_ptr_->lateral;
    // out.longitudinal = manual_cmd_ptr_->longitudinal;
  }
  control_cmd_pub_->publish(out);
  // RCLCPP_INFO(get_logger(), "publish cmd");
