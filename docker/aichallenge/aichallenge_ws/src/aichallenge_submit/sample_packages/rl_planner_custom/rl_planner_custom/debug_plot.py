...
class PlotMarker():
    ...
    def plot_status(self,
        ...
        if ego_pose is not None:
            ...
            ## Plot ego pose
            plot_xmin = ego_pose[0] - self.plot_width / 4.0
            ...
            ego_rot_inv = np.array([[math.cos(yaw), -math.sin(yaw)],
                                    [math.sin(yaw), math.cos(yaw)]])
            ego_rot_inv_wh = np.array([[math.cos(yaw), 0],
                                       [0, math.sin(yaw)]])
            outline = np.array([
            ...
            if rotation is None:
                ...
                ego_vec = np.array([np.cos(0), np.sin(0)]) * self.ego_vec_length
            ...
            if object_pose is not None:
                for ob in object_pose:
                    ...
                    if rotation:
                        ...
                        object_xy = object_xy[0:2] @ ego_rot_inv
                        object_wh = object_wh[0:2] @ ego_rot_inv
