...
void RawVehicleCommandConverterNode::publishActuationCmd()
{
  ...
  // }
  // if (brake_flag_) {
  //   desired_accel_cmd = 0.0;
  //   desired_brake_cmd = 0.2;
  //   RCLCPP_INFO(get_logger(), "accel: %.2f, brake: %.2f", desired_accel_cmd, desired_brake_cmd);
  // }

  actuation_cmd.actuation.accel_cmd = desired_accel_cmd;  // 1.0;
  actuation_cmd.actuation.brake_cmd = desired_brake_cmd;  // 0.0;
