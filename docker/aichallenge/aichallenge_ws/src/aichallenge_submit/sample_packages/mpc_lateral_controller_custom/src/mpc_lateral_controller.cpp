...
MpcLateralController::MpcLateralController(rclcpp::Node & node) : node_{&node}
{
  ...
  if (vehicle_model_type == "kinematics") {
    ...
    // // NOLINT
    vehicle_model_ptr =
      std::make_shared<DynamicsBicycleModel>(wheelbase, mass_fl, mass_fr, mass_rl, mass_rr, cf, cr, m_mpc.m_param.steer_tau);
  } else {
  ...
  trajectory_follower::InputData const & input_data)
{
  ...
  if (m_current_kinematic_state.twist.twist.linear.x > 10 / 3.6) {
    is_solved_ = m_mpc.calculateMPC(
